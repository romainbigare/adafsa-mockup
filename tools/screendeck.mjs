...
const WORK = join(ROOT, '.deck-work', 'screens');   // each deck builder owns one folder; tourdeck.mjs has its own
...
// ---------------------------------------------------------------- browser --
/* Serving, the map-tile relay and the browser live in lib/stage.mjs, shared
 * with the bilingual tour deck. */
await mkdir(WORK, { recursive: true });
const stage = await openStage({ viewport: VIEW, scale: SCALE });
const { page, tileStats } = stage;
const settleTiles = stage.settleTiles;
...
await stage.close();
stopOnProblems({ problems: stage.problems, tileFailures: [] });
...
stopOnProblems({ problems: [], tileFailures: stage.tileFailures });
